var operationTime = {
    ...
    //两个时间戳之间的时间段
    calculatePeriod2: function(lang1, lang2) {
      var period = lang2 - lang1;
      period = Math.ceil(period / 60000);
      if (period >= 0 && period < 60) {
        return {
          minute:period<10?'0'+period:period,
          hour:'00'
        }
      } else if (period >= 60) {
        return {
          minute:period % 60<10?'0'+period % 60:period % 60,
          hour:parseInt(period / 60)<10?'0'+parseInt(period / 60):parseInt(period / 60)
        }
      } else {
        return "数据错误"
      }
    ...
    calcHeight ( refbox ) {
    	...
    	var ele = document.getElementById(refbox);
...
export function creditorTypes(type) {
    if(type == '01'){
      type = '车主'
    } else if(type == '02'){
      type = '乘客'
